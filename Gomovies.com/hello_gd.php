...
disp($rate4);*/
	...
	// here we are using thegradient descent 
    $m = count($y);
	if($m==0)
	{
	...
	$theta = array(get_rand(0,1), get_rand(0,1), get_rand(0,1), get_rand(0,1), get_rand(0,1));
	$theta_final = gradientDescentMulti($x , $y, $theta, 0.0003,1500);
...
$json_arr=array('mae'=>round($mae,2),'recommended'=>$recommended);
	echo json_encode($json_arr);
?>